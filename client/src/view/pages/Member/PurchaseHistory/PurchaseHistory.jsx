...
function PurchaseHistory({orders}) {
        ...
        productsList = productsArray.map(product => (
            <ListItem key={Math.floor(Math.random() * 100000)} className={classes.purchaseItem}>
                ...
                <Box className={classes.itemInfoBlock}>
                    <p className={classes.purchaseItemdate}>{product.date.replace('-', '/').split('T')[0].replace('-', '/')}</p>
                    ...
                    <p className={classes.purchaseItemAddInfo}>Total price ${product.currentPrice} USD</p>
                </Box>
            </ListItem>
        ))
        :
        productsList = <Typography
    ...
    return (
        <>
            <Box data-testid="purchase-history-list">
                {productsList}
            </Box>
        </>
    );
